...
The major-cycle sequence runs forward (advancing in $\Z_{60}$) iff the year stem and the native have matching polarity---a yang-year (even stem) with a male, or a yin-year (odd stem) with a female (the traditional rule \emph{yang-male, yin-female run forward}); otherwise it runs backward. (With $y=1$ when the year stem is yang and $s=1$ when the native is male, this is the single test $y=s$.) The starting age is the number of days from the birth to the next (forward) or previous (backward) solar term, divided by three.
\end{proposition}
